...
if(!dir.exists("philawhite-spGDMM-code-d8279ee/data/")) {
  download_zenodo("10.5281/zenodo.10091441")
  unzip("spGDMM-code-spGDMM_v1.zip")
}

Data <- read.csv("philawhite-spGDMM-code-d8279ee/data/sa_species_data.csv")
...
SA.Formula <- formula(
  paste0("Abund ~ f(Species, hyper = PCHyper) + f(Family, hyper = PCHyper) + f(plot, hyper = PCHyper) + ", 
         paste(
...
Use <- !names(Posts)%in%paste0("Precision.for.", c("species", "site"))
